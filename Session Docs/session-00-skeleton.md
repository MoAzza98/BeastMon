...
Run these commands before writing any files:

```bash
git checkout main
git pull origin main
git checkout -b session/00-skeleton
```

When all files are created and the verification checklist passes:
...
npm test
git add .
git commit -m "chore(skeleton): scaffold monorepo with all stubs and configs"
git push origin session/00-skeleton
```
